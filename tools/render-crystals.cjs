...
/* ⚠️ full היא הסצנה שנשלחת לדף, ובה c=0 בכוונה.
   ...
   onlyB/onlyA/noA נשארו לכלי הקומפוזיט הדו-ממדי. */
const LAYERS = {
  full:  { a: 1, b: 1, c: 0 },
  noA:   { a: 0, b: 1, c: 1 },
  onlyB: { a: 0, b: 1, c: 0 },
...
/* ============================================================
   הגביש השמאלי (a) — חייב להיכנס במלואו למסגרת
   ------------------------------------------------------------
   קודם הוא ישב ב---ax()*1.00 ונחתך בקצה השמאלי: הקצה המחודד
   שלו פשוט לא היה בתמונה. הלקוח ביקש להשלים אותו (27.8).
   ...
   ה-extent הוא האלכסון sqrt(L²+r²), כדי שגם בסיבוב הגרוע
   ביותר הגביש עדיין נכנס.

   ⚠️ שני חיתוכים ולא אחד. מלבד מסגרת התמונה, object-fit:cover
   בדפדפן חותך את ההפרש בין יחס התמונה ליחס תיבת ה-hero:
     1920x1080 → 22.5% מהגובה בכל צד
     1440x900  → 13.3% מהגובה
     834       →  7.3% מהרוחב
     360       → 15.5% מהרוחב
   מכאן שני האילוצים:
     • kA מוגבל ל-0.70 ברוחב, כי גובה הגביש חייב להיכנס לתוך
       55% מגובה התמונה ששורדים ב-1920.
     • mgA הוא 0.15 קבוע — מספיק לכסות את החיתוך האופקי הגרוע
       ביותר בשתי התמונות.
   ⚠️ אם משנים את גובה ה-hero או את יחס התמונה, למדוד מחדש.
   ============================================================ */
float halfW(){ return 0.5 * uFull.x / uFull.y; }
float kA(){ return clamp(0.30 + 0.25*(uFull.x/uFull.y), 0.42, 0.70); }
vec2  szA(){ return vec2(2.10, 0.86) * kA(); }
float eA(){ vec2 z=szA(); return sqrt(z.x*z.x + z.y*z.y) * 1.55 / 9.40; }
float mgA(){ return 0.15; }
/* 🔑 גובה הגביש תלוי ביחס התמונה.
   ברוחב שולחני הוא **חייב** לשבת במרכז האנכי: ב-1920x1080
   object-fit:cover משאיר רק 55% מגובה התמונה, וגביש שאמור
   להיכנס במלואו אין לו לאן לזוז.
   במסגרת לאורך אין חיתוך אנכי בכלל, ולכן יש מקום — ושם הוא
   יורד מתחת לגוש הטקסט. זה מה שמאפשר להשאיר את הגביש בהיר
   בנייד במקום להחשיך אותו עד שהוא נעלם. */
float uyA(){ return 0.55 * clamp(1.2 - uFull.x/uFull.y, 0.0, 1.0); }
vec3 pA(){
  float ux = -(halfW() - eA() - mgA());
  return vec3(ux * 9.40 / 1.55, uyA() * 9.40 / 1.55, 0.40);
}
